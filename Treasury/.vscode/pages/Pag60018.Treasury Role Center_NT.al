page 60018 "Treasury Role Center_NT"
{
    Caption = 'Treasury', Comment = 'Use same translation as ''Profile Description'' (if applicable)';
    PageType = RoleCenter;
    ...
    layout
    {
        area(rolecenter)
        {
            ...
            //     ApplicationArea = all;
            // }
            part(Control1902304208; "Treasury Activities_NT")
            {
                ApplicationArea = all;
            }
    ...
    actions
    {
        ...
        area(embedding)
        {
            ...
            action("&Statements")
            {
                ...
                ToolTip = 'Record your store statement';
            }
            action("&TreasuryStmts")
            {
                ApplicationArea = all;
                Caption = 'Treasury Statements';
                Image = CashFlow;
                RunObject = Page "Treasury Statement List_NT";
                ToolTip = 'Record your treasury statement';
            }
            action("&PostedTreasuryStmts")
            {
                ApplicationArea = all;
                Caption = 'Posted Treasury Statements';
                RunObject = Page "Posted Treasury Stmt. List_NT";
                ToolTip = 'View treasury statements that are posted.';
            }
            action(PostedStatements)
            {
                ApplicationArea = all;
                Caption = 'Posted Statements';
                RunObject = Page "LSC Posted Statement List";
                ToolTip = 'View statements that are posted.';
            }
            action("&TransactionRegister2")
            {
                ApplicationArea = all;
        ...
        area(sections)
        {
            ...
            group(Journals)
            {
                ...
                action(TreasuryJnl)
                {
                    ...
                    ToolTip = 'Register expenses through treasury journal.';
            ...
            group("Posted Documents")
            {
                ...
                // }
                action("Posted Treasury Statements")
                {
                    ApplicationArea = all;
                    Caption = 'Posted Treasury Statements';
                    RunObject = Page "Posted Treasury Stmt. List_NT";
                    ToolTip = 'View auditing details for all posted treasury statement.';
                }
                action(PostedStmts)
                {
                    ApplicationArea = all;
                ...
                action("Posted Treasury Journal")
                {
                    ApplicationArea = all;
                    Caption = 'Posted Treasury Journal';
                    RunObject = Page "Posted Treasury Journal_NT";
                    ToolTip = 'View auditing details for all treasury statement entries.';
                }
            }
#if not CLEAN18
        ...
        area(creation)
        {
            ...
            // }
            action("Treasury Payment Journal")
            {
                //AccessByPermission = TableData "Treasury Journal Batch_NT" = imd;
                ApplicationArea = all;
                Caption = 'Treasury Payment Entry';
                RunObject = Page "Treasury Journal Batches";
                RunPageView = where("Jnl. Entry Type" = filter("Cash-Payments"));
                ToolTip = 'Prepare to post payment transaction to the company books.';
            }
            action("Treasury Receipt Journal")
            {
                //AccessByPermission = TableData "Treasury Journal Batch_NT" = imd;
                ApplicationArea = all;
                Caption = 'Treasury Receipt Entry';
                RunObject = Page "Treasury Journal Batches";
                RunPageView = where("Jnl. Entry Type" = filter("Cash-Receipts"));
                ToolTip = 'Prepare to post receipt transaction to the company books.';
            }
